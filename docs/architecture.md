...
- **`init()`** -- Registers all 42 tools with the MCP server
...
All 42 tools are registered in the `init()` method of `AcumaticaMcpServer`. Each tool has:
